...
function generatePrivKey() {
    ...
    vscode.window.showQuickPick(keyLengths)
        ...
        openssl_1.default.genPrivKey(value.size, value.algo).then(data => {
...
function getKeyCsrGenerator(extensionPath) {
    return () => {
        ...
        panel.webview.onDidReceiveMessage((message) => {
            // validate input
            openssl_1.default.genKeyCsr(message)
                .then((data) => {
                vscode.workspace.openTextDocument({
                    content: data.key
                }).then(doc => {
...
function getSelfSignedCertGenerator(extensionPath) {
    return () => {
        let bsCssUri = vscode.Uri.file(path.join(extensionPath, 'assets', 'css', 'bootstrap.min.css'));
        bsCssUri = bsCssUri.with({ scheme: 'vscode-resource' });
        let extCssUri = vscode.Uri.file(path.join(extensionPath, 'assets', 'css', 'opensslutils.css'));
        ...
        panel.webview.html = html;
        panel.webview.onDidReceiveMessage((message) => {
            openssl_1.default.genSelfSignedCert(message)
                .then((data) => {
                vscode.workspace.openTextDocument({
                    content: data.key
                }).then(doc => {
...
function getP12Generator(extensionPath) {
    return () => {
            ...
            if (message.command === 'export') {
                vscode.window.showSaveDialog({
                    ...
                    if (res) {
                        ...
                        openssl_1.default.genP12(message).then(() => {
